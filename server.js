...
app.get('/health', (_, res) => res.json({ status: 'ok', rooms: rooms.size, lobbies: Object.ke
...
players.map(p => ({ id: p.id, name: p.name, hand: [], calledU
this.deck this.direction = 1;
...
for (let i = 0; i < 2; i++) {
for (const n of NUMBERS.slice(1)) this.deck.push({ color, value: n, type: 'nu
for (const a of ACTIONS) this.deck.push({ color, value: a, type: 'a
}
}
for (let i = 0; i < 4; i++) {
...
return card.type === 'wild' || card.color === this.currentColor || card.value === thi
}
...
(card.type === 'number' && card.color === this.currentColor && card.value ===
);
...
const active = this.activePlayers().filter(p => p.id !== this.players[playerI
...
if (lastCard && (lastCard.color === targetColor || lastCard.type === 'wil
}
...
type: '0',
unoTransfer: this.players[nextIdx].hand.length === 1 ? this.playe
: this.players[playerIndex].hand.length === 1 ? this.playe
: undefined
};
}
this.advanceTurn();
...
if (this.isMercy()) {
let guard = 0;
while (this.knockedOut.includes(this.players[next]?.id) && guard++ < this.players
...
swapHands(a, b) {
[this.players[a].hand, this.players[b].hand] = [this.players[b].hand, this.players[a]
}
chooseSwapTarget(playerId, targetId) {
if (!this.pendingSwap7) return { success: false, error: 'No swap
if (this.pendingSwap7.playerId !== playerId) return { success: false, error: 'Not yo
...
if (pi === ti) return { success: false, error: 'Cannot swap with yoursel
this.swapHands(pi, ti);
this.pendingSwap7 = null;
this.advanceTurn();
return { success: true, swapperName: this.players[pi].name, targetName: this.players[
}
...
drawCard(playerId) {
const pi = this.players.findIndex(p => p.id === playerId);
if (pi === -1 || pi !== this.currentPlayer) return { success: false, error: 'Not your
if (this.hasDrawnThisTurn) return { success: false, error: 'You can
...
let guard = 0;
while (this.knockedOut.includes(this.players[this.currentPlayer]?.id) && guard++
this.advanceTurn();
}
}
return newlyKnocked;
...
let guard = 0;
while (this.knockedOut.includes(this.players[next]?.id) && guard++ < this.players.len
next = (next + this.direction + this.players.length) % this.players.length;
}
return next;
}
...
if (player.hand.length !== 1) return { success: false, error: 'Can only call O,No whe
...
if (caught.hand.length !== 1 || caught.calledUno) return { success: false, error: 'No
this.drawCards(cui, 2);
caught.calledUno = false;
return { success: true, catcherName: this.players[ci].name, caughtName: caught.name,
}
checkUnoAfterPlay(playerIndex) {
if (this.players[playerIndex].hand.length !== 1) this.players[playerIndex].calledUno
}
reshuffleDeck() {
...
const pm = new PlayerPresenceManager(2, { heartbeatInterval: 5000, reconnectTimeout:
...
socket.join(id);
socket.emit('lobbyCreated', { roomId: id, lobbyName, settings, players: lobbies[id].p
broadcastLobbyList();
});
socket.on('joinLobby', ({ lobbyId, playerName, passcode }) => {
const lobby = lobbies[lobbyId];
if (!lobby) return;
if (lobby.players.some(p => p.id === socket.id)) { socket.emit('error', 'You ar
if (lobby.players.length >= lobby.settings.maxPlayers) { socket.emit('error', 'Lobby
if (lobby.isPrivate && passcode !== lobby.passcode) { socket.emit('error', ' Wron
...
socket.join(lobbyId);
socket.emit('lobbyJoined', { roomId: lobbyId, lobbyName: lobby.name, settings: io.to(lobbyId).emit('lobbyUpdate', { roomId: lobbyId, players: lobby.players });
broadcastLobbyList();
lobby.
});
...
io.to(roomId).emit('error', `Need at least ${minPlayers} players to start (cu
return;
}
...
io.to(roomId).emit('lobbyUpdate', { roomId, players: lobby.players });
io.to(roomId).emit('error', 'Some players disconnected before game start. Please
return;
}
if (pm) {
...
console.log(`[Server] Starting game in lobby ${roomId} with ${lobby.players.length} p
...
return ps && ps.state !== PlayerState.TIMEOUT && ps.state !== PlayerState
});
if (active.length < minPlayers) {
io.to(roomId).emit('gameEnded', { reason: 'Not enough players remaining',
rooms.delete(roomId); pm.destroy(); lobbyPresenceManagers.delete(roomId);
} else {
io.to(roomId).emit('playerTimeout', { playerId: timedOutPlayer.id, player
}
});
}
...
socket.emit('chooseSwapTarget', { opponents: room.players.filter(p => p.id broadcastGameState(room);
return;
!== so
}
...
io.to(roomId).emit('unoTransfer', { playerName: result.swapHappened.unoTransf
}
...
io.to(roomId).emit('drawAnimation', { victimId: socket.id, victimName: p?.name, p
}
// Mercy: check if someone got knocked out from drawing
if (room.isMercy && room.isMercy()) {
const knocked = room.checkMercyKnockouts();
knocked.forEach(p => io.to(roomId).emit('playerKnockedOut', { playerId: p.id, pla
...
io.to(roomId).emit('gameOver', { winner: active[0].name, winnerId: active[0].
rematchQueues.set(roomId, { players: room.players.map(p=>({id:p.id,name:p.nam
setTimeout(() => rematchQueues.delete(roomId), 60000);
rooms.delete(roomId);
return;
}
...
if (result.cardsDrawn) state.lastDrawInfo = { cardsDrawn: result.cardsDrawn, play
...
if (!result.success) { socket.emit('error', result.error); return; }
io.to(roomId).emit('swapHappened', { swapperName: result.swapperName, targetName: res
broadcastGameState(room);
});
...
if (result.success) {
io.to(roomId).emit('playerCalledUno', { playerName: result.playerName, message: `
broadcastGameState(room);
}
});
...
io.to(roomId).emit('unoPenalty', { catcherName: result.catcherName, caughtName: r
broadcastGameState(room);
...
const missing = q.players.filter(p => !io.sockets.sockets.get(p.id)).map(p =>
q.players.filter(p => io.sockets.sockets.get(p.id))
.forEach(p => io.to(p.id).emit('rematchCancelled', { reason: `${missing}
return;
}
...
(p.name.toLowerCase() === (playerName || '').toLowerCase() && !room.players.find(
);
if (playerIdx === -1) {
socket.emit('rejoinFailed', { reason: 'Player not found in this game. Make return;
sure y
}
...
console.log(`[Server] ${room.players[playerIdx].name} rejoined room ${roomId} (${oldI
});
...
io.to(p.id).emit('rematchCancelled', { reason: `${decliner?.name || 'A pl
});
}
});
...
.forEach(p => io.to(p.id).emit('playerDisconnected', { playerId: socket.i
}
});
broadcastLobbyList();
});
