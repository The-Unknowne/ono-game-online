...
const MAX_DRAW_ATTEMPTS = 50; // Safety limit for "draw until match" to prevent infinite loop
/* =======================
STATIC FILES & ROUTES
======================= */
...
// Determine draw penalty BEFORE processing (for animation)
// We need to know: who gets penalized and how many cards
let drawPenaltyTarget = null;
let drawPenaltyCount = 0;
if (!this.settings.allowStacking) {
if (card.value === '+2') {
drawPenaltyTarget = this.players[this.getNextPlayerIndex(playerIndex)];
drawPenaltyCount = 2;
...
// Animation data: who gets penalized, by whom, how many cards
drawPenalty: drawPenaltyTarget ? {
targetPlayerId: drawPenaltyTarget.id,
targetPlayerName: drawPenaltyTarget.name,
count: drawPenaltyCount,
...
if (this.stackedDrawCount > 0) {
const stackCount = this.stackedDrawCount;
this.drawCards(playerIndex, this.stackedDrawCount);
this.stackedDrawCount = 0;
this.stackStartedBy = null;
this.advanceTurn();
return { success: true, drewStacked: true, stackCount };
}
...
io.to(roomId).emit('lobbyUpdate', { roomId: roomId, players: lobby.players })
io.to(roomId).emit('error', 'Some players disconnected before game start. Ple
return;
}
...
io.to(roomId).emit('error', `Cannot start: Players not responding: console.log(`[Server] Game start prevented - missing players: ${missingNa
return;
${miss
}
}
console.log(`[Server] Starting game in lobby ${roomId} with ${lobby.players.lengt
...
return pm && pm.state !== PlayerState.TIMEOUT && pm.state !== PlayerS
});
if (activePlayers.length < 3) {
// Not enough players, end game
io.to(roomId).emit('gameEnded', {
reason: 'Not enough players remaining',
message: `${timedOutPlayer.name} disconnected and did not reconne
});
rooms.delete(roomId);
...
message: `${timedOutPlayer.name} has been removed from the game d
});
}
});
}
...
// Not enough players
io.to(roomId).emit('error', `Need at least 3 players to start (currently ${lobby.
}
});
...
// ─── DRAW ANIMATION EVENT ─────────────────────────────────────────────
// Emit BEFORE gameState so clients can start the animation first,
// then receive the new state once the visual completes.
if (result.drawPenalty) {
const penalty = result.drawPenalty;
// Tell the room about the incoming draw animation
io.to(roomId).emit('drawPenaltyAnimation', {
targetPlayerId: penalty.targetPlayerId,
targetPlayerName: penalty.targetPlayerName,
count: penalty.count,
playedByName: penalty.playedByName,
cardValue: penalty.cardValue,
// Animation delay in ms — give clients time to animate before updating state
stateDelay: 800
});
}
// ─────────────────────────────────────────────────────────────────────
// Delay the game state update slightly when there's a draw animation
// so the animation has time to play before the hand updates
const stateDelay = result.drawPenalty ? 800 : 0;
...
message: `${result.unoPenalty.playerName} forgot to call O,No! Drawing 2
});
}
// Check for winner
...
if (stateDelay > 0) {
setTimeout(broadcastState, stateDelay);
} else {
broadcastState();
}
});
...
// If player drew stacked cards (+2/+4 accumulated), emit draw animation for them
if (result.drewStacked && result.stackCount) {
io.to(roomId).emit('drawPenaltyAnimation', {
targetPlayerId: socket.id,
targetPlayerName: null, // client knows who they are
count: result.stackCount,
playedByName: null,
cardValue: null,
isStackedDraw: true,
stateDelay: 800
...
setTimeout(broadcastDrawState, 800);
} else {
broadcastDrawState();
}
});
...
message: `${result.catcherName} caught ${result.caughtName}! ${result.caughtN
});
// Broadcast updated game state to all players
room.players.forEach(p => {
io.to(p.id).emit('gameState', room.getGameState(p.id));
